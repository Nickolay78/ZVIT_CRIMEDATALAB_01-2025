...
raw_f22016_3 <- read_excel(fileload, 
                            sheet = "2.3", 
                            range = special_pgo$Trange3[ij], 
                            col_names = namesc2, 
                            col_types = colT2)
  
  raw_f22016_4 <- read_excel(fileload, 
                             sheet = "2.4", 
                             range = special_pgo$Trange4[ij], 
                             col_names = namesc2, 
                             col_types = colT2)


  r_f2_1<-rbind(raw_f22016_1,raw_f22016_2)
  r_f2_2<-rbind(raw_f22016_3,raw_f22016_4)
...
if (ij %in% 5:9)# 9 means 2024 year
{
...
write.csv(test_extract,file=paste0("DATAF2_2/",2015+ij,"-f2_2.csv"), row.names = FALSE)
if (ij==1) main_f2_2<-test_extract else main_f2_2<-rbind(main_f2_2,test_extract)

}

#write.xlsx(names(test_extract),"extra2.xlsx")
save (main_f2_2,file = "f2_2.RData")
